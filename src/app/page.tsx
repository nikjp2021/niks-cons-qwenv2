...
export default function Home() {
  ...
  return (
    <>
        ...
        <div className="relative z-10 text-center section-container" style={{ maxWidth: '900px' }}>
          ...
          <FadeIn delay={1}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-5 mb-24">
              <MagneticButton
                href="/get-a-quote"
                className="glow-button group inline-flex items-center justify-center gap-3 px-12 py-6 font-semibold rounded-full text-lg"
              >
                Get a Free Quote
              ...
              <MagneticButton
                href="/services"
                className="inline-flex items-center justify-center gap-2 px-12 py-6 border border-[var(--surface-4)]/50 text-[var(--text-200)] font-semibold rounded-full text-lg hover:border-[var(--surface-5)] hover:bg-[var(--surface-1)] transition-all duration-[var(--duration-normal)]"
              >
                Explore Services
              </MagneticButton>
            </div>
      ...
      {/* ═══════════════════════════════════════════════════
          SERVICES — Premium grid with glass cards
          ═══════════════════════════════════════════════════ */}
      ...
      <section className="section-padding border-y border-[var(--surface-3)]/20 overflow-hidden">
        <div className="section-container">
          <div className="grid lg:grid-cols-[1fr_1.2fr] gap-20 items-center">
            <ScrollReveal>
              <span className="badge-pill mb-4 inline-flex">Integrated Ecosystem</span>
              ...
              <p className="text-[var(--text-300)] leading-relaxed mb-12 text-lg font-light">
              ...
              <div className="flex flex-wrap gap-4">
                {tools.map((tool) => (
                  <span
                    key={tool.name}
                    className="inline-flex items-center gap-2.5 px-5 py-3 glass-card text-[var(--text-300)] text-sm font-medium"
                  >
                    {toolIcons[tool.icon]}
                    {tool.name}
                  </span>
      ...
      {/* ═══════════════════════════════════════════════════
          PROCESS — Premium 4-step with connecting line
          ═══════════════════════════════════════════════════ */}
      <section className="section-padding relative overflow-hidden">
        ...
        <div className="relative section-container">
          <ScrollReveal className="mb-20 max-w-2xl">
            <span className="badge-pill mb-4 inline-flex">Our Process</span>
            <h2 className="font-display font-extrabold text-5xl sm:text-6xl tracking-[-0.03em] text-[var(--text-100)]">
              How we work
            </h2>
            <p className="text-xl text-[var(--text-300)] mt-6 leading-relaxed font-light">
          ...
          <StaggerContainer className="relative" staggerDelay={0.12}>
            {/* Connecting line */}
            <div className="hidden lg:block absolute top-16 left-[10%] right-[10%] h-px bg-gradient-to-r from-transparent via-[var(--surface-4)]/30 to-transparent" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
              {processSteps.map((step) => (
                <StaggerItem key={step.step}>
                  <div className="relative">
                    <div
                      className="w-14 h-14 rounded-2xl flex items-center justify-center mb-10 border-2"
                      style={{
                    ...
                    <span className="font-mono text-xs font-semibold block mb-4 tracking-wider" style={{ color: step.color }}>
                      {step.step}
                    </span>
                    <h3 className="font-display font-bold text-2xl mb-4 text-[var(--text-100)]">
                      {step.title}
      ...
      {/* ═══════════════════════════════════════════════════
          CASE STUDIES — Premium featured results
          ═══════════════════════════════════════════════════ */}
      <section className="section-padding bg-[var(--surface-1)]/30 relative overflow-hidden">
        <GradientOrb color="rgba(244, 63, 94, 0.04)" size={600} top="-200px" right="-200px" />

        <div className="relative section-container">
          <ScrollReveal className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-8 mb-20">
            <div className="max-w-2xl">
              <span className="badge-pill mb-4 inline-flex">Case Studies</span>
              <h2 className="font-display font-extrabold text-5xl sm:text-6xl tracking-[-0.03em] text-[var(--text-100)]">
                Success stories
              </h2>
              <p className="text-xl text-[var(--text-300)] leading-relaxed mt-6 font-light">
      ...
      <section className="section-padding relative overflow-hidden">
        ...
        <div className="relative section-container">
          <ScrollReveal className="mb-20 text-center">
            ...
            <p className="text-xl text-[var(--text-300)] mt-6 max-w-2xl mx-auto font-light">
            ...
            {testimonials.map((t) => (
              <StaggerItem key={t.id}>
                <div className="glass-card p-8 h-full flex flex-col">
                  ...
                  <Quote className="w-10 h-10 text-[var(--color-brand-500)]/15 mb-6" />
                  <p className="text-[var(--text-300)] leading-relaxed mb-8 flex-1 text-lg">
          ...
          <ScrollReveal>
            <div className="glass-card p-12 sm:p-16 lg:p-24 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-5 justify-center mb-14">
                <MagneticButton
                  href="/get-a-quote"
                  className="glow-button group inline-flex items-center justify-center gap-3 px-10 py-5 font-semibold rounded-full text-lg text-white"
                >
                  Get a Free Quote
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </MagneticButton>
                <MagneticButton
                  href="/contact"
                  className="inline-flex items-center justify-center gap-2 px-10 py-5 border border-[var(--surface-4)]/50 text-[var(--text-200)] font-semibold rounded-full text-lg hover:border-[var(--surface-5)] transition-all duration-[var(--duration-normal)]"
                >
                  Get in Touch
                </MagneticButton>
              </div>
